...
def svr(pain_tree_root, nopain_tree_root, save_root):
    ...
    save_root = save_root + '/train.train'
    print("save_root")
    nopain_child = []
    child = []
    ...
    for name_count in range(5):
        ...
        for i in range(len(child[3])):
            ...
            x = (int(child[name_count + 3][i].attrib['x']) + int(child[name_count + 3][i].attrib['width']) * 0.5)
            y = (int(child[name_count + 3][i].attrib['y']) + int(child[name_count + 3][i].attrib['height']) * 0.5)
    ...
    for name_count in range(5):
        ...
        for i in range(len(nopain_child[3])):
            x = (int(nopain_child[name_count + 3][i].attrib['x']) + int(
                nopain_child[name_count + 3][i].attrib['width']) * 0.5)
            y = (int(nopain_child[name_count + 3][i].attrib['y']) + int(
                nopain_child[name_count + 3][i].attrib['height']) * 0.5)
            aaa.append([x, y])
    ...
    for i in range(len(no_pain[0])):
        health.append([no_pain[0][i], no_pain[1][i], no_pain[2][i], no_pain[3][i], no_pain[4][i], no_pain[5][i],
                       no_pain[6][i],
                       no_pain[7][i], no_pain[8][i], no_pain[9][i]])
